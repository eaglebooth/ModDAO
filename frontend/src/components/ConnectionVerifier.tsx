...
type ContractState = { initialized: string; admin: string; treasury_available: string; treasury_reserved: string; total_funded: string; total_paid: string; total_withdrawn: string; mod_count: string; eval_count: string; settlement_count: string };
...
export function ConnectionVerifier(){
  ...
  async function verify(){if(!address)return;setBusy(true);setMessage("Reading get_contract_state from Studionet...");const result=await readContract("get_contract_state",[],address);if(result.success&&typeof result.data==="string"){try{setState(JSON.parse(result.data));setMessage("Contract state refreshed successfully from ModDAO V3.");}catch{setMessage("The address responded, but the contract state payload was malformed.");}}else setMessage(result.error||"Contract read failed.");setBusy(false)}
